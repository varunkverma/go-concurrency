...
func main() {
	...
	clientChannel := make(chan string, seatingCapacity) // maximum 10 channels in this channel
	...
	go func() {
		<-time.After(timeOpen) // it will block till the time is up
		shopClosing <- true
	...
	go func() {
		for {
			// get a random number with arrival avergae rate
	...
	// block unitl the barbershop is closed
	<-closed
	close(shopClosing)
	close(closed)
}
